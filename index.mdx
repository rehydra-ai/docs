...
<CardGroup cols={3}>
  ...
  <Card title="OpenCode Plugin" icon="shield" href="/guides/opencode-plugin">
    Prevent coding agents from leaking secrets from `.env` files.
  </Card>
</CardGroup>
...
Use `@rehydra/opencode` when you want OpenCode to scrub secrets before they are sent to an LLM.

- Scan `.env` values locally
- Replace detected secrets with placeholders in prompts and tool inputs
...
<CardGroup cols={2}>
  ...
  <Card title="OpenCode Plugin Guide" icon="shield" href="/guides/opencode-plugin">
    Set up automatic secret scrubbing for OpenCode.
  </Card>
</CardGroup>
