...
| `ACCOUNT_NUMBER` | Generic account numbers | `ACC-12345678` | Configurable pattern |
| `TAX_ID` | Tax identification numbers | `DE123456789` | Configurable pattern |
| `NATIONAL_ID` | National ID numbers | `SSN 123-45-6789` | Configurable pattern |
...
────────────────────────────────────────────────────────────────────────────────────→
URL → IP → LOCATION → ORG → PERSON → PHONE → EMAIL → IBAN → CREDIT_CARD → Custom IDs
```

Higher priority types take precedence when spans overlap. Custom ID patterns (CASE_ID, CUSTOMER_ID) take precedence over generic types.

## Detection Sources

Each detected entity has a `source` indicating how it was found:

| Source | Description |
|--------|-------------|
| `REGEX` | Detected via regex pattern matching |
| `NER` | Detected via NER model inference |
| `HYBRID` | Detected by both regex and NER (overlapping spans merged) |
